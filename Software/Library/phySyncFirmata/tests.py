...
from phySyncFirmata import mockup
from phySyncFirmata.boards import BOARDS
from phySyncFirmata.util import (
...
class BoardBaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        phySyncFirmata.phySyncFirmata.serial.Serial = mockup.MockupSerial
        # Set the wait time to a zero so we won't have to wait a couple of secs
        # each test
        phySyncFirmata.phySyncFirmata.BOARD_SETUP_WAIT_TIME = 0
        self.board = phySyncFirmata.Board('', BOARDS['arduino'])
...
class TestBoardMessages(BoardBaseTest):
    ...
    def test_incoming_analog_message(self):
        ...
        self.board.sp.write([phySyncFirmata.ANALOG_MESSAGE + 4, 127, 7])
        self.board.iterate()
        self.assertEqual(self.board.analog[4].read(), None)
    ...
    def test_incoming_digital_message(self):
        ...
        self.board.digital[9].mode = phySyncFirmata.INPUT
        ...
        self.board.sp.write([phySyncFirmata.DIGITAL_MESSAGE + 1, mask % 128, mask >> 7])
        self.board.iterate()
    ...
    def test_incoming_report_version(self):
        self.assertEqual(self.board.firmata_version, None)
        self.board.sp.write([phySyncFirmata.REPORT_VERSION, 2, 1])
        self.board.iterate()
        self.assertEqual(self.board.firmata_version, (2, 1))
    ...
    def test_incoming_report_firmware(self):
        ...
        msg = [phySyncFirmata.START_SYSEX,
               phySyncFirmata.REPORT_FIRMWARE,
               2,
               1] + list(str_to_two_byte_iter('Firmware_name')) + \
              [phySyncFirmata.END_SYSEX]
    ...
    def test_report_digital(self):
        ...
        self.board.digital[8]._mode = phySyncFirmata.INPUT  # Outputs can't report
...
class TestBoardLayout(BoardBaseTest):
    ...
    def test_layout_arduino_mega(self):
        phySyncFirmata.phySyncFirmata.serial.Serial = mockup.MockupSerial
        mega = phySyncFirmata.Board('', BOARDS['arduino_mega'])
    ...
    def test_pwm_layout(self):
        ...
        for pin in pins:
            self.assertEqual(pin.mode, phySyncFirmata.PWM)
    ...
    def test_get_pin_digital(self):
        ...
        self.assertEqual(pin.mode, phySyncFirmata.OUTPUT)
    ...
    def tearDown(self):
        self.board.exit()
        phySyncFirmata.serial.Serial = serial.Serial
    ...
    def test_handle_digital_inputs(self):
        ...
        for i in range(8, 16):  # pins of port 1
            if not bool(i % 2) and i != 14:  # all even pins
                self.board.digital[i].mode = phySyncFirmata.INPUT
